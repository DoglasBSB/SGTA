...
$pdf->Cell(1, 0, "                                        Relatório de Alunos", 0, 1, 'L');
$pdf->Ln(1);
$pdf->SetFont('Arial', '', 12);
$pdo = new PDO('mysql:host=localhost; dbname=sgta', 'root', '');
$sql = $pdo->prepare("SELECT * FROM tb_aluno ");
$sql->execute();

foreach ($sql as $resultado) {

    $pdf->Cell(0, 0.5, 'Nome: ' . $resultado['nome'], 0, 1, 'L');
    $pdf->Cell(0, 0.5, 'CPF: ' . $resultado['cpf'], 0, 1, 'L');
    $pdf->Cell(0, 0.5, 'Matrícula: ' . $resultado['matricula'], 0, 1, 'L');
    $pdf->Cell(0, 0.5, 'Telefone: ' . $resultado['telefone'], 0, 1, 'L');
